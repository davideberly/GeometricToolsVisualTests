...
bool Delaunay3DWindow3::SetEnvironment()
{
    ...
    if (path != "")
    {
        mEnvironment.Insert(path + "/GTL/Samples/Mathematics/Geometry/Delaunay3D/Data/");
        return true;
    }
    else
    {
...
bool Delaunay3DWindow3::CreateScene()
{
    ...
    auto extremes = ComputeExtremes(mVertices);
    Vector3<float> vmin = extremes.first, vmax = extremes.second;
    for (int32_t j = 0; j < 3; ++j)
    {
...
{
    std::vector<size_t> const& dindices = mDelaunay.GetIndices();
    ...
    Vector4<float> const& oppositeColor)
{
    ...
    size_t vOpposite = mInfo.finalV[3];
