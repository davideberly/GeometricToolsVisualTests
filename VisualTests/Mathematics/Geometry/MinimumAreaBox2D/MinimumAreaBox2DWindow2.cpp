    ...
    mHull{}
{
    ...
    {
        float angle = rnd(mte) * C_TWO_PI<float>;
...
bool MinimumAreaBox2DWindow2::SetEnvironment()
{
    ...
    mEnvironment.Insert(path + "GTL/VisualTests/Mathematics/Geometry/MinimumAreaBox2D/Data/");

    if (mEnvironment.GetPath("convexpolygon.txt") == "")
    {
        GTL_RUNTIME_ERROR("Cannot find file convexpolygon.txt");
        return false;
    }

    if (mEnvironment.GetPath("projection.raw") == "")
    {
        GTL_RUNTIME_ERROR("Cannot find file projection.raw");
        return false;
    }
