...
void NURBSCurveExampleWindow2::DoSimulation1()
{
    ...
    size_t imax = mSpline->GetNumControls();
...
void NURBSCurveExampleWindow2::DoSimulation2()
{
    ...
    else
    {
        ...
        size_t imax = mCircle->GetNumControls();
        for (size_t i = 0; i < imax; ++i)
        {
...
void NURBSCurveExampleWindow2::InitialConfiguration()
{
    ...
    BasisFunction<float>::Input input(numControls, 2);
    mSpline = std::make_shared<NURBSCurve<float, 2>>(input, mControls.data(), weights.data());
}
...
void NURBSCurveExampleWindow2::NextConfiguration()
{
    ...
    BasisFunction<float>::Input input0(5, 2);
    mSpline = std::make_shared<NURBSCurve<float, 2>>(input0, mControls.data(), weights.data());

    BasisFunction<float>::Input input1;
    input1.numControls = 11;
    input1.degree = 2;
    input1.uniform = true;
    input1.periodic = true;
    input1.uniqueKnots.resize(input1.numControls + input1.degree + 1);
    float invNmD = 1.0f / static_cast<float>(input1.numControls - input1.degree);
    for (size_t i = 0; i < input1.uniqueKnots.size(); ++i)
    {
        int32_t diff = static_cast<int32_t>(i) - static_cast<int32_t>(input1.degree);
        input1.uniqueKnots[i].t = static_cast<float>(diff) * invNmD;
        input1.uniqueKnots[i].multiplicity = 1;
    }
    mCircle = std::make_shared<NURBSCurve<float, 2>>(input1, mControls.data() + 5, weights.data() + 5);
}
