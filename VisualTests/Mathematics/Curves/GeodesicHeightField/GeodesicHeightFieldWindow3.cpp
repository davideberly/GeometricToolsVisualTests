    ...
    mCurvature(0.0)
{
    ...
    size_t const numIntegralSamples = 16;
    size_t const numSearchSamples = 32;
    size_t const numSubdivisions = 6;
    size_t const numRefinements = 1;
    double const derivativeStep = 1e-04;
    double const searchRadius = 0.1;
    mGeodesic = std::make_unique<BSplineGeodesic<double>>(
        mSurface, numIntegralSamples, numSearchSamples, numSubdivisions,
        numRefinements, derivativeStep, searchRadius);
    mGeodesic->refinementCallback = [this]()
    {
        size_t currQuantity = mGeodesic->GetCurrentQuantity();
        if (currQuantity == 0)
        {
    ...
    mPathQuantity = (1ull << mGeodesic->GetNumSubdivisions()) + 1;
}
...
void GeodesicHeightFieldWindow3::OnIdle()
{
    ...
    if (mSelected == 2)
    {
        ...
        for (int32_t i = 1; i < currQuantity; ++i)
        {
            ...
            Rasterize2<int32_t>::DrawLine(x0, y0, x1, y1, mDrawCallback);
            x0 = x1;
            y0 = y1;
        }
    ...
    size_t substep = mGeodesic->GetSubdivision();
    size_t refstep = mGeodesic->GetRefinement();
...
bool GeodesicHeightFieldWindow3::SetEnvironment()
{
    ...
    mEnvironment.Insert(path + "GTL/VisualTests/Mathematics/Curves/GeodesicHeightField/Data");

    if (mEnvironment.GetPath("ControlPoints.txt") == "")
    {
        GTL_RUNTIME_ERROR("Cannot find file ControlPoints.txt");
        return false;
    }
    return true;
}
...
void GeodesicHeightFieldWindow3::CreateScene()
{
    ...
    std::array<size_t, 2> numControls{}, degree{};
    inFile >> numControls[0];
    inFile >> numControls[1];
    inFile >> degree[0];
    inFile >> degree[1];

    std::array<BasisFunction<double>::Input, 2> input{};
    ...
    mSurface = BSplineSurface<double, 3>(input, nullptr);
    ...
    for (size_t i = 0; i < numControls[0]; ++i)
    {
        double u = dXExtent * (-1.0 + 2.0 * static_cast<double>(i) / invN0Cm1);
        for (size_t j = 0; j < numControls[1]; ++j)
        {
    ...
    auto vertices = vbuffer->Get<Vertex>();
    std::array<Vector<double, 3>, 6> jet{};
    for (uint32_t i = 0; i < numVertices; ++i)
    {
        ...
        position[2] = static_cast<float>(mSurface.GetPosition(u, v)[2]);
    }
