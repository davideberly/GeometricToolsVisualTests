...
GeodesicEllipsoidWindow2::GeodesicEllipsoidWindow2(Parameters& parameters)
    ...
    mParam1(2),
    mXMin(0.0f),
    mXMax(C_PI_DIV_2<float>),
    mXDelta((mXMax - mXMin) / static_cast<float>(mSize)),
    mYMin(C_PI_DIV_2<float> / static_cast<float>(mSize)),
    mYMax(C_PI_DIV_2<float>),
    mYDelta((mYMax - mYMin) / static_cast<float>(mSize)),
    mNumTruePoints(129),
    mTruePoints(mNumTruePoints),
    mNumApprPoints((1 << mGeodesic.GetNumSubdivisions()) + 1),
    ...
    mApprCurvature(0.0f)
{
    ...
    {
        mTruePoints[i].resize(2);
    }

    for (int32_t i = 0; i < mNumApprPoints; ++i)
    {
        mApprPoints[i].resize(2);
    }
...
void GeodesicEllipsoidWindow2::OnDisplay()
{
    ...
    size_t numApprPoints = mGeodesic.GetCurrentQuantity();
    if (numApprPoints == 0)
    {
...
void GeodesicEllipsoidWindow2::DrawScreenOverlay()
{
    ...
    size_t currquan = mGeodesic.GetCurrentQuantity();
...
void GeodesicEllipsoidWindow2::ComputeTruePath()
{
    ...
    std::uniform_real_distribution<float> rnd(0.0f, C_PI_DIV_2<float>);
    mParam0[0] = rnd(dre);
    mParam0[1] = rnd(dre);
    mParam1[0] = rnd(dre);
    mParam1[1] = rnd(dre);
...
void GeodesicEllipsoidWindow2::ComputeApprPath(bool subdivide)
{
    if (subdivide)
    {
        size_t newNumApprPoints = 2 * mCurrNumApprPoints - 1;
        ...
        for (size_t i = mCurrNumApprPoints - 1; i > 0; --i)
        {
            size_t twoI = 2 * static_cast<size_t>(i);
            mApprPoints[twoI] = mApprPoints[i];
        }

        for (size_t i = 0; i <= mCurrNumApprPoints - 2; ++i)
        {
            size_t twoI = 2 * static_cast<size_t>(i);
    ...
    {
        for (size_t i = 1; i <= mCurrNumApprPoints - 2; ++i)
        {
...
void GeodesicEllipsoidWindow2::ParamToXY(Vector<float> const& param, int32_t& x, int32_t& y) const
{
...
void GeodesicEllipsoidWindow2::XYToParam(int32_t x, int32_t y, Vector<float>& param) const
{
