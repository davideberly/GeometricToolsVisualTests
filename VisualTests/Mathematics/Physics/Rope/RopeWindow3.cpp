...
void RopeWindow3::OnIdle()
{
    ...
    auto delta = std::chrono::duration_cast<std::chrono::milliseconds>(mTime1 - mTime0).count();
    if (120 * delta >= 1000)
    {
        mTime0 = mTime1;
        PhysicsTick();
...
bool RopeWindow3::OnCharPress(std::uint8_t key, std::int32_t x, std::int32_t y)
{
    switch (key)
    {
    ...
    case 'm':  // decrease mass
        ...
        {
            for (std::size_t i = 1; i + 1 < mModule->GetNumParticles(); ++i)
            {
    ...
    case 'c':  // decrease spring constant
        ...
        {
            for (std::size_t i = 0; i < mModule->GetNumSprings(); ++i)
            {
...
bool RopeWindow3::SetEnvironment()
{
    ...
    {
        GTL_RUNTIME_ERROR("Cannot find file Rope.png.");
        return false;
    }
...
{
    std::size_t constexpr numParticles = 8;
    float constexpr step = 0.1f;
    ...
    for (std::size_t i = 1; i + 1 < numParticles; ++i)
    {
    ...
    std::size_t constexpr numSprings = numParticles - 1;
    float restLength = 1.0f / static_cast<float>(numSprings);
    for (std::size_t i = 0; i < numSprings; ++i)
    {
...
{
    Mesh<float>::Description desc(Mesh<float>::Topology::CYLINDER, 64, 8);
    ...
    BasisFunction<float>::Input input(mModule->GetNumParticles(), 2);
    mSpline = std::make_shared<BSplineCurve<float, 3>>(input, &mModule->GetPosition(0));
...
void RopeWindow3::PhysicsTick()
{
    ...
    std::size_t numControls = mModule->GetNumParticles();
