// Geometric Tools Library
// https://www.geometrictools.com
// Copyright (c) 2025 Geometric Tools LLC
// Distributed under the Boost Software License, Version 1.0
// https://www.boost.org/LICENSE_1_0.txt
// File Version: 0.0.2025.10.15

#include "PhysicsModule.h"

PhysicsModule::PhysicsModule(std::size_t numParticles, float step,
    Vector3<float> const& gravity, Vector3<float> const& wind,
    float windChangeAmplitude, float viscosity)
    :
    MassSpringCurve<float, 3>(numParticles, step),
    enableWind(false),
    enableWindChange(false),
    mGravity(gravity),
    mWind(wind),
    mWindChangeAmplitude(windChangeAmplitude),
    mViscosity(viscosity),
    mDRE{},
    mURD(-1.0f, 1.0f)
{
}

Vector3<float> PhysicsModule::ExternalAcceleration(std::size_t i, float const&,
    std::vector<Vector3<float>> const&, std::vector<Vector3<float>> const& velocity)
{
    // Acceleration due to gravity.
    Vector3<float> acceleration = mGravity;

    // Acceleration due to wind.
    if (enableWind)
    {
        if (enableWindChange)
        {
            // Generate random direction close to last one.
            std::array<Vector3<float>, 3> basis{};
            basis[0] = mWind;
            float length = Normalize(basis[0]);
            ComputeOrthonormalBasis(1, basis[0], basis[1], basis[2]);
            float uDelta = mWindChangeAmplitude * mURD(mDRE);
            float vDelta = mWindChangeAmplitude * mURD(mDRE);
            basis[0] += uDelta * basis[1] + vDelta * basis[2];
            Normalize(basis[0]);
            mWind = length * basis[0];
        }
        acceleration += mWind;
    }

    // Add in a friction term; otherwise the system tends to be stiff, in the
    // sense of numerical stability, and leads to oscillatory behavior.
    acceleration -= mViscosity * velocity[i];

    return acceleration;
}
