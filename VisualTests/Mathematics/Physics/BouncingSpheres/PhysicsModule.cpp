...
PhysicsModule::PhysicsModule(std::size_t numSpheres, double xMin, double xMax,
...
void PhysicsModule::InitializeSphere(std::size_t i, double radius, double massDensity,
...
void PhysicsModule::DoCollisionDetection()
{
    ...
    std::size_t const numSpheres = mRigidSphere.size();
    ...
    for (std::size_t i0 = 0; i0 + 1 < numSpheres; ++i0)
    {
        auto const& sphere0 = mRigidSphere[i0]->GetWorldSphere();

        for (std::size_t i1 = i0 + 1; i1 < numSpheres; ++i1)
        {
...
void PhysicsModule::DoCollisionResponse(double time, double deltaTime)
{
    ...
    for (std::size_t i = 0; i < mRigidSphere.size(); ++i)
    {
