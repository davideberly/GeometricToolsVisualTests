...
{
public:
    PhysicsModule(std::size_t numSpheres, double xMin, double xMax, double yMin,
        double yMax, double zMin, double zMax);

    // This function must be called for each of the numSpheres sphere objects
    // before starting the simulation.
    void InitializeSphere(std::size_t i, double radius, double massDensity,
    ...
    inline std::size_t GetNumSpheres() const
    {
    ...
    inline Plane3<double> GetPlane(std::size_t i) const
    {
    ...
    inline Sphere3<double> GetWorldSphere(std::size_t i) const
    {
