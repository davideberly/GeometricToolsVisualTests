...
bool GelatinBlobWindow3::OnCharPress(std::uint8_t key, std::int32_t x, std::int32_t y)
{
    switch (key)
    {
    case 'w':
...
void GelatinBlobWindow3::CreateSprings()
{
    ...
    for (std::size_t i = 0; i < numParticles / 2; ++i)
    {
        mModule->SetMass(i, 1.0f);
        mModule->SetPosition(i, vertices[i].position);
        mModule->SetVelocity(i, { urd(dre), urd(dre), urd(dre) });
    }
    for (std::size_t i = numParticles / 2; i < numParticles; ++i)
    {
        mModule->SetMass(i, fmax);
        mModule->SetPosition(i, 2.0f * vertices[i - numParticles / 2].position);
    ...
    std::uint32_t const numTriangles = ibuffer->GetNumPrimitives();
    auto indices = ibuffer->Get<std::uint32_t>();
    for (std::uint32_t t = 0; t < numTriangles; ++t)
    {
        std::uint32_t v0 = *indices++;
        std::uint32_t v1 = *indices++;
        std::uint32_t v2 = *indices++;
    ...
    float constexpr constant = 10.0f;
    std::size_t index = 0;
    MassSpringArbitrary<float, 3>::Spring spring{};
    for (auto const& edge : edgeSet)
    {
        spring.particle0 = edge[0];
        spring.particle1 = edge[1];
    ...
    {
        spring.particle0 = i;
        spring.particle1 = i + numParticles / 2;
        spring.constant = constant;
        spring.length = Length(mModule->GetPosition(spring.particle1)
            - mModule->GetPosition(spring.particle0));
...
void GelatinBlobWindow3::CreateSegments()
{
    ...
    for (std::size_t index = 0; index < numSprings; ++index)
    {
        MassSpringArbitrary<float, 3>::Spring spring = mModule->GetSpring(index);
...
void GelatinBlobWindow3::PhysicsTick()
{
    ...
    for (std::size_t index = 0; index < numSprings; ++index)
    {
        ...
        auto segment = std::static_pointer_cast<Visual>(
            mSegmentRoot->GetChild(static_cast<std::int32_t>(index)));
