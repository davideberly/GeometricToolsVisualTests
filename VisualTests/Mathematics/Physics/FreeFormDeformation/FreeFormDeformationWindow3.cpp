...
void FreeFormDeformationWindow3::CreateBSplineVolume()
{
    ...
    {
        BasisFunction<float>::Input(quantity, degree),
        BasisFunction<float>::Input(quantity, degree),
        BasisFunction<float>::Input(quantity, degree)
    };

    mSpline = std::make_unique<NURBSVolume<float, 3>>(input, nullptr, nullptr);

    // Compute a bounding box of the form [xmin,xmax]x[ymin,ymax]x[zmin,zmax].
    auto const& vbuffer = mMesh->GetVertexBuffer();
    std::uint32_t const numVertices = vbuffer->GetNumElements();
    ...
    Vector3<float> range = mMax - mMin, control{};
    mDelta = range / static_cast<float>(quantity - 1);
    for (std::size_t i2 = 0; i2 < quantity; ++i2)
    {
        control[2] = mMin[2] + mDelta[2] * i2;
        for (std::size_t i1 = 0; i1 < quantity; ++i1)
        {
            control[1] = mMin[1] + mDelta[1] * i1;
            for (std::size_t i0 = 0; i0 < quantity; ++i0)
            {
                control[0] = mMin[0] + mDelta[0] * i0;
                mSpline->SetControl(i0, i1, i2, control);
            }
        }
    }

    // Generate some weights, not all 1, to visualize the effect on the
    // free-form mesh.
    float weight{};
    for (std::size_t i2 = 0; i2 < quantity; ++i2)
    {
        for (std::size_t i1 = 0; i1 < quantity; ++i1)
        {
            for (std::size_t i0 = 0; i0 < quantity; ++i0)
            {
                ...
                mSpline->SetWeight(i0, i1, i2, weight);
            }
        }
    }

    // Compute the (u,v,w) values of the mesh relative to the NURBS-spline
    // volume.
...
void FreeFormDeformationWindow3::CreateSegments()
{
    ...
    for (std::size_t i2 = 0; i2 < quantity; ++i2)
    {
        for (std::size_t i1 = 0; i1 < quantity; ++i1)
        {
            for (std::size_t i0 = 0; i0 < quantity - 1; ++i0)
            {
                ...
                vertices[0] = mSpline->GetControl(i0, i1, i2);
                vertices[1] = mSpline->GetControl(i0 + 1, i1, i2);
    ...
    for (std::size_t i2 = 0; i2 < quantity; ++i2)
    {
        for (std::size_t i0 = 0; i0 < quantity; ++i0)
        {
            for (std::size_t i1 = 0; i1 < quantity - 1; ++i1)
            {
                vbuffer = std::make_shared<VertexBuffer>(vformat, 2);
                vbuffer->SetUsage(Resource::Usage::DYNAMIC_UPDATE);
                vbuffer->SetUsage(Resource::Usage::DYNAMIC_UPDATE);
                vertices = vbuffer->Get<Vector3<float>>();
                vertices[0] = mSpline->GetControl(i0, i1, i2);
                vertices[1] = mSpline->GetControl(i0, i1 + 1, i2);
    ...
    for (std::size_t i1 = 0; i1 < quantity; ++i1)
    {
        for (std::size_t i0 = 0; i0 < quantity; ++i0)
        {
            for (std::size_t i2 = 0; i2 < quantity - 1; ++i2)
            {
                vbuffer = std::make_shared<VertexBuffer>(vformat, 2);
                vbuffer->SetUsage(Resource::Usage::DYNAMIC_UPDATE);
                vertices = vbuffer->Get<Vector3<float>>();
                vertices[0] = mSpline->GetControl(i0, i1, i2);
                vertices[1] = mSpline->GetControl(i0, i1, i2 + 1);
    ...
    for (std::size_t i2 = 0; i2 < quantity; ++i2)
    {
        for (std::size_t i1 = 0; i1 < quantity; ++i1)
        {
            for (std::size_t i0 = 0; i0 < quantity; ++i0)
            {
                ...
                box->localTransform.SetTranslation(mSpline->GetControl(i0, i1, i2));
                box->Update();
...
void FreeFormDeformationWindow3::UpdateMesh()
{
    ...
    {
        Vector3<float> param = mParameters[i];
        mSpline->Evaluate(param[0], param[1], param[2], 0, values);
...
void FreeFormDeformationWindow3::UpdateBoxes()
{
    ...
    for (std::size_t i2 = 0; i2 < quantity; ++i2)
    {
        for (std::size_t i1 = 0; i1 < quantity; ++i1)
        {
            for (std::size_t i0 = 0; i2 < quantity; ++i2)
            {
                auto box = std::static_pointer_cast<Visual>(mControlRoot->GetChild(i));
                Vector3<float> control = mSpline->GetControl(i0, i1, i2);
                box->localTransform.SetTranslation(control);
                ++i;
            }
        }
    }
    ...
    for (std::size_t i2 = 0; i2 < quantity; ++i2)
    {
        ...
        for (std::size_t i1 = 0; i1 < quantity; ++i1)
        {
            ...
            for (std::size_t i0 = 0; i0 < quantity; ++i0)
            {
                ...
                Vector3<float> newControl = mSpline->GetControl(i0, i1, i2) +
                    mAmplitude * Vector3<float>{ urd(dre), urd(dre), urd(dre) };

                Vector3<float> diff = newControl - control;
                ...
                mSpline->SetControl(i0, i1, i2, control + diff);
            }
        }
    }
...
void FreeFormDeformationWindow3::OnMouseMove(int32_t x, int32_t y)
{
    ...
    mSelected->localTransform.SetTranslation(
        mSelected->localTransform.GetTranslation() + HProject(localDiff));
    mSelected->Update();
    ...
    mSpline->SetControl(i0, i1, i2, mSelected->localTransform.GetTranslation());
