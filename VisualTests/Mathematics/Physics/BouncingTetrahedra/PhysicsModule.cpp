...
PhysicsModule::PhysicsModule(std::size_t numTetra, double xMin,
...
void PhysicsModule::InitializeTetrahedron(std::size_t i, double massDensity,
    ...
    Vector3<double> const& angularVelocity)
{
    ...
    mRigidTetra[i]->Force = [this, i](double, RigidBodyState<double> const& state)
    {
        ...
        std::size_t numVerticesOnFloor = 0;
...
void PhysicsModule::DoCollisionDetection()
{
    ...
    // simulation region.
    std::size_t const numTetra = mRigidTetra.size();
    std::vector<bool> moved(numTetra, false);
    double overlap{};
    for (std::size_t i = 0; i < numTetra; ++i)
    {
    ...
    for (std::size_t i0 = 0; i0 + 1 < numTetra; ++i0)
    {
        auto const& sphere0 = mRigidTetra[i0]->GetWorldSphere();

        for (std::size_t i1 = i0 + 1; i1 < numTetra; ++i1)
        {
...
void PhysicsModule::DoCollisionResponse(double time, double deltaTime)
{
    ...
    for (std::size_t i = 0; i < mRigidTetra.size(); ++i)
    {
    ...
    Contact& contact)
{
    ...
    std::size_t numZeroBary0 = 0, numZeroBary1 = 0;
    std::size_t indicator0 = 0, indicator1 = 0;
    for (std::size_t i = 0, mask = 1; i < 4; ++i, mask <<= 1)
    {
