...
{
public:
    PhysicsModule(std::size_t numTetra, double xMin, double xMax, double yMin,
    ...
    void InitializeTetrahedron(std::size_t i,
        double massDensity,
    ...
    inline Plane3<double> const& GetPlane(std::size_t i) const
    {
    ...
    inline Tetrahedron3<double> const& GetBodyTetrahedron(std::size_t i) const
    {
        return mRigidTetra[i]->GetBodyTetrahedron();
    }

    inline Tetrahedron3<double> GetWorldTetrahedron(std::size_t i) const
    {
    ...
    // The input must satisfy 0 <= i < numTetrahedra where the upper bound was
    // passed to the constructor.
    inline Sphere3<double> const& GetWorldSphere(std::size_t i) const
    {
        return mRigidTetra[i]->GetWorldSphere();
    }

    inline Vector3<double> const& GetPosition(std::size_t i) const
    {
        return mRigidTetra[i]->GetPosition();
    }

    inline Matrix3x3<double> const& GetOrientation(std::size_t i) const
    {
