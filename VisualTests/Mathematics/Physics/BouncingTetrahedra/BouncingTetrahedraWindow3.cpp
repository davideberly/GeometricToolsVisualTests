    ...
    mSingleStep(false)
{
    ...
    // Initialize the tetrahedra with the correct transformations.
    PhysicsTick();
    GraphicsTick();
}
...
bool BouncingTetrahedraWindow3::OnCharPress(std::uint8_t key, std::int32_t x, std::int32_t y)
{
    switch (key)
    {
    case 'w':
...
bool BouncingTetrahedraWindow3::SetEnvironment()
{
    ...
    mEnvironment.Insert(path + "GTL/VisualTests/Mathematics/Physics/BouncingTetrahedra/Data/");
...
void BouncingTetrahedraWindow3::CreatePhysicsObjects()
{
    ...
    for (std::size_t i = 0; i < NUM_TETRAHEDRA; ++i)
    {
        input >> radius;
...
void BouncingTetrahedraWindow3::CreateGraphicsObjects()
{
    ...
    std::array<std::size_t, 12> faceIndices = Tetrahedron3<double>::GetAllFaceIndices();
    for (std::size_t j = 0; j < 12; ++j)
    {
        indices[j] = static_cast<std::uint32_t>(faceIndices[j]);
    }
...
void BouncingTetrahedraWindow3::CreateWall(
    std::size_t index, VertexFormat const& vformat,
    ...
    Vector4<float> const& color)
{
    ...
    auto ibuffer = std::make_shared<IndexBuffer>(IPType::IP_TRIMESH, 2, sizeof(std::uint32_t));
