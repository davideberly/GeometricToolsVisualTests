...
bool PickingWindow3::SetEnvironment()
{
    ...
    {
        GTL_RUNTIME_ERROR(
            "Cannot open file Checkerboard.png.");
        return false;
    }
...
void PickingWindow3::DoPick(int32_t x, int32_t y)
{
    ...
    if (mCamera->GetPickLine(viewX, viewY, viewW, viewH, x, y, origin, direction))
    {
        ...
        if (mNumActiveSpheres > 0)
        {
            ...
            Matrix4x4<float> const& invWMatrix = mScene->worldTransform.GetInverseH();
            for (int32_t i = 0; i < mNumActiveSpheres; ++i)
            {
                Vector3<float> modelPosition = HProject(invWMatrix * mPicker.records[i].primitivePoint);
